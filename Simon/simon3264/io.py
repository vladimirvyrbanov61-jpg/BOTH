...
from simon3264.features import blocks_to_feature_matrix
...
def save_blocks_npz(
    ...
    *,
    ...
    """Save blocks and optional labels/keys to .npz."""
...
def load_blocks_npz(path: str | Path) -> dict[str, np.ndarray]:
    """Load .npz written by save_blocks_npz."""
    with np.load(path, allow_pickle=False) as data:
        return {k: data[k] for k in data.files}


def _parse_hex_line(line: str) -> np.ndarray:
    line = line.strip()
    ...
    if len(raw) == BLOCK_BYTES:
        return bytes_to_block(raw)
    if len(raw) % BLOCK_BYTES == 0:
        return bytes_to_blocks(raw)[0]
    raise ValueError(f"hex line length {len(raw)} not multiple of {BLOCK_BYTES}")
...
def blocks_from_file(
    ...
    *,
    ...
    """Load ciphertext blocks from disk.

    npz: expects 'blocks' array (N, 2).
    bin: raw concatenated 4-byte blocks.
    hex: one block per line (8 hex chars or 4 byte tokens).
    """
    path = Path(path)
    if format == "npz":
...
def decrypt_check(
    ...
    """Decrypt and optionally verify plaintext; return residual metrics."""
...
def blocks_to_feature_file(
    ...
    *,
    ...
    """Load blocks from file, validate, return feature matrix; optionally save .npz."""
