"""Block representation: words, bytes, and bit vectors (SIMON 32/64)."""
...
def validate_block(words: np.ndarray) -> bool:
    """Return True if *words* is a valid 32-bit SIMON block (2 x uint16)."""
    w = np.asarray(words)
    if w.shape != (2,):
        return False
    for x in w.flat:
...
def block_to_bytes(
    ...
    """Pack [left, right] uint16 words into 4 bytes.

  little (default, refinery / blog vectors): word0 then word1, each LE uint16.
  big: network-order uint16 words.
    """
...
def bytes_to_block(
    ...
    """Unpack 4 bytes into shape (2,) uint16 [left, right]."""
...
def bytes_to_blocks(
    ...
    """Unpack concatenated 4-byte blocks into shape (N, 2)."""
...
def block_to_bits(
    ...
    """Expand one block to a length-2n bit vector (MSB-first per word: left then right)."""
    w = np.asarray(words, dtype=np.uint16).reshape(2)
...
def blocks_to_bits(blocks: np.ndarray, n: int = WORD_BITS) -> np.ndarray:
    """Shape (N, 2) words -> (N, 2*n) bit matrix."""
