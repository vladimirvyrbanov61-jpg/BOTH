#!/usr/bin/env python3
"""Run full cryptanalysis assignment pipeline: train AE (if needed) + both tracks + report."""
...
def ensure_torch_model(model_dir: Path, *, quick: bool = False) -> None:
    ...
    if model_path.exists():
        ...
        return
    print("[run_all] Training base TorchAutoencoder via ml/train.py --torch-only ...")
...
def main(argv=None) -> None:
    parser = argparse.ArgumentParser(description="Run assignment cryptanalysis experiments")
    parser.add_argument("--config", default=None)
    parser.add_argument(
        "--quick",
        action="store_true",
        help="Use configs/cryptanalysis_quick.yaml and faster AE training",
    )
    parser.add_argument("--skip-train", action="store_true", help="Do not auto-train AE")
