...
def test_cryptanalysis_config_loads():
    ...
    assert 32 in cfg.round_sweep.round_values
...
def test_distinguisher_train_smoke():
    ...
    loss0 = model._train_loss_history[0]
    loss_last = model._train_loss_history[-1]
    assert loss_last <= loss0 * 1.5 + 0.1
...
def test_round_sweep_smoke():
    ...
    blocks32, labels32 = generate_round_dataset(cipher, 32, 20, rng)
    assert int(labels32[0]) == 0
...
def test_round_sweep_csv_smoke(tmp_path):
    ...
    run_round_sweep(cfg, quick_rounds=[8, 32], model_dir=model_dir)
