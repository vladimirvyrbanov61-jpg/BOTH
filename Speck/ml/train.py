#!/usr/bin/env python3
"""Train anomaly detection models on SPECK 32/64 ciphertext features."""
...
def main(argv: Optional[list[str]] = None) -> None:
    parser = argparse.ArgumentParser(description="Train SPECK 32/64 anomaly models.")
    ...
    print("SPECK 32/64 Anomaly Detection — Training")
    print(f"  n_samples={cfg.data.n_samples}  anomaly_fraction={cfg.data.anomaly_fraction:.1%}")

    ds = generate_or_load_dataset(cfg, cache_dir=Path(cfg.paths.data_dir), force_regen=args.force_regen)
    split = DataSplit(ds)
    print(split.summary())
